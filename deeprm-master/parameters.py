...
class Parameters:
    def __init__(self):
        ...
        self.output_filename ='data/pg_re'   #'data/tmp'

        self.num_epochs =20 #10000(原始)        # number of training epochs
        self.simu_len = 10             # length of the busy cycle that repeats itself
        self.num_ex = 10               # number of sequences

        self.num_machine = 5          # number of computer集群机器数量
        ...
        self.time_horizon = 20         # number of time steps in the graph 时间步长
        self.max_job_len = 15          # maximum duration of new jobs
        self.res_slot = 20             # maximum number of available resource slots
        self.max_job_size = 20         # maximum resource request of new work

        self.backlog_size = 60         # backlog queue size   预选数据  排队
    ...
    def compute_dependent_parameters(self):
        ...
        self.network_output_dim = self.num_nw + 1  # + 1 for void action  所有可调度的作业
